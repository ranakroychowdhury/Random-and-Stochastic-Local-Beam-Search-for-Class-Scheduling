...
/**
 * Created by Ranakrc on 12-Nov-17.
 */
public class Schedule {
    public static int teachers, rooms, classes, periods = 30, requirements, k = 4, count = 100;
    ...
    //Finding k elements with random costs
    //STOCHASTIC
    public static void randomStates(ArrayList<State> node) {
        ...
        while (i < k) {
            ...
            int n = rand.nextInt(node.size());
            if (!isPresent(kElements, n)) {
                kElements[i] = n;
                i++;
            }
        }
    ...
    public static State localBeamSearch() {
        ...
        for(stoch = 0; stoch < count; stoch++) {
            ...
            for(i = 0; i < k; i++) {
                ...
                globalCost[i] = initStates.get(i).cost;
            }
            ...
            randomStates(childStates);
            //smallestStates(childStates);
            for(i = 0; i < k; i++) {
                initStates.remove(i);
                initStates.add(childStates.get(kElements[i]));
                //printState(initStates.get(i));
            }
        }
